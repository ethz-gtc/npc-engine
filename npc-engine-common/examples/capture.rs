...
use npc_engine_common::{
    ...
    Domain, IdleTask, MCTSConfiguration, StateDiffRef, StateDiffRefMut, Task, TaskDuration, MCTS,
};
...
impl Behavior<CaptureGame> for AgentBehavior {
    fn add_own_tasks(
        ...
        if let Some(agent_state) = agent_state {
            ...
            let other_tasks: Vec<Box<dyn Task<CaptureGame>>> =
    ...
    fn get_state_description(state_diff: StateDiffRef<Self>) -> String {
        ...
        for (id, state) in &state.agents {
            if let Some(target) = state.next_location {
                ...
                s += &format!(
                    "\nA{} @    {}, ❤️ {}, • {}, ⚡{}",
                    id.0, state.cur_or_last_location.0, state.hp, state.ammo, state.acc_capture
                );
            }
        }
        s
...
impl ExecutorState<CaptureGame> for CaptureGameExecutorState {
    fn post_mcts_run_hook(
        &mut self,
        mcts: &MCTS<CaptureGame>,
        last_active_task: &ActiveTask<CaptureGame>,
    ) {
...
fn main() {
    ...
    run_simple_executor::<CaptureGame, CaptureGameExecutorState>(&CONFIG, &mut executor_state);
}
