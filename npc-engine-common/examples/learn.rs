...
impl Task<LearnDomain> for Collect {
	fn duration(&self, _tick: u64, _state_diff: StateDiffRef<LearnDomain>, _agent: AgentId) -> TaskDuration {
		1
	}
...
impl Behavior<LearnDomain> for DefaultBehaviour {
	...
	fn is_valid(&self, _tick: u64, _state_diff: StateDiffRef<LearnDomain>, _agent: AgentId) -> bool {
		true
	}
}
...
impl Domain for LearnDomain {
	...
	fn update_visible_agents(_start_tick: u64, _tick: u64, _state_diff: StateDiffRef<Self>, agent: AgentId, agents: &mut std::collections::BTreeSet<AgentId>) {
        agents.insert(agent);
    }

	fn get_state_description(state_diff: StateDiffRef<Self>) -> String {
		let state = Self::get_cur_state(state_diff);
		let map = state.map.iter()
...
impl StateValueEstimator<LearnDomain> for NNStateValueEstimator {
    fn estimate(
		...
		let state = LearnDomain::get_cur_state(
...
fn main() {
	...
	for _epoch in 0..600 {
		run_simple_executor::<LearnDomain, LearnExecutorState>(
			&CONFIG,
			&mut executor_state
		);
		let wood_collected = TOTAL_WOOD as f32 - executor_state
			.planned_values
			.last()
			.unwrap().0
			.iter()
		...
		executor_state.estimator.0.train(
			executor_state.planned_values.iter(),
			0.001
		);
